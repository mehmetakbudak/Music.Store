...
function getAll() {
  axios
    .post(`http://localhost:8544/api/Customer/GetByFilter`, filter)
    .then((res: any) => {
      data.value = res.data;
      loading.value = false;
    });
...
const edit = (id: number) => {
  ...
  axios.get(`http://localhost:8544/api/Customer/${id}`).then((res: any) => {
    Object.assign(form, res.data);
  });
};

const remove = (id: number) => {
  axios.delete(`http://localhost:8544/api/Customer/${id}`).then((res: any) => {
    loading.value = true;
    getAll();
    ElNotification({
      type: "success",
...
const save = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      if (form.id == 0) {
        axios
          .post(`http://localhost:8544/api/Customer`, form)
          .then((res: any) => {
            loading.value = true;
        ...
        axios
          .put(`http://localhost:8544/api/Customer`, form)
          .then((res: any) => {
            loading.value = true;
            modalVisible.value = false;
            getAll();
